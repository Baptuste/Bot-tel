...
// Ids de statut portant un sens métier, tirés de `features.orderFlow`.
const FULFILLED_ID = requireRole('fulfilled').id;
const CANCELLED_ID = requireRole('cancelled').id;
const PLACED_ID = requireRole('placed').id;
  ...
  if (!_activeRoutes) {
    _activeRoutes = db.prepare(`
      SELECT r.id, r.time_slot AS label, r.date, d.name AS driver,
        COUNT(o.id)                              AS total,
        COALESCE(SUM(o.status = ?), 0)           AS delivered
      FROM routes r
...
export function getDashboard(): DashboardData {
  ...
  const today = q.today.get(FULFILLED_ID, CANCELLED_ID, FULFILLED_ID) as DashboardData['today'];

  const pending = (
    q.pending.all(PLACED_ID) as Array<{
      id: number;
  ...
  const activeRoutes = features.deliverySlots.enabled
    ? (activeRoutesStmt().all(FULFILLED_ID) as DashboardData['activeRoutes'])
    : [];
...
export function getOverduePendingIds(minutes = PENDING_ALERT_MINUTES): number[] {
  return (q.overdue.all(PLACED_ID, minutes) as Array<{ id: number }>).map((r) => r.id);
}
